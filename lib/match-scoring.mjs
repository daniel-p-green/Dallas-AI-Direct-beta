...
export function calculateOverlapScore(helpNeeded, helpOffered) {
...
export function calculateComfortProximityScore(sourceComfort, candidateComfort) {
  if (!Number.isFinite(sourceComfort) || !Number.isFinite(candidateComfort)) {
    return 0;
  }

  const distance = Math.min(MAX_COMFORT_DISTANCE, Math.abs(sourceComfort - candidateComfort));
  return roundScore(1 - distance / MAX_COMFORT_DISTANCE);
}

export function calculateConsentVisibilityScore(candidate) {
  if (!candidate || candidate.display_title_company !== true) {
    return 0;
  }

  const hasTitleOrCompany = Boolean(candidate.title?.trim() || candidate.company?.trim());
  return hasTitleOrCompany ? 1 : 0;
}

export function calculateRecencyScore(candidateCreatedAt, now, recencyWindowDays = DEFAULT_RECENCY_WINDOW_DAYS) {
...
export function calculateMatchScore(sourceAttendee, candidateAttendee, options = {}) {
  ...
  const recencyScore = calculateRecencyScore(
    candidateAttendee.created_at,
    options.now ?? new Date().toISOString(),
    options.recencyWindowDays,
  );
 ...
 * 3) candidate id asc
 */
export function rankMatches(sourceAttendee, candidates, options = {}) {
  return candidates
    .filter((candidate) => candidate?.id && candidate.id !== sourceAttendee.id)
    .map((candidate) => {
      const score = calculateMatchScore(sourceAttendee, candidate, options);
      return {
